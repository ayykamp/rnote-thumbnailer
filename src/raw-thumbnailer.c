...
save_pixbuf (GdkPixbuf *pixbuf, const char *path, int size)
{
  ...
  if (jpeg_output) {
    saved = gdk_pixbuf_save (small, path, "jpeg", &err, NULL);
  }
  else {
    a_width = g_strdup_printf ("%d", width);
    a_height = g_strdup_printf ("%d", height);
    saved = gdk_pixbuf_save (small, path,
                             "png", &err,
...
#endif /* THUMB_DEBUG */
  { G_OPTION_REMAINING, '\0', 0, G_OPTION_ARG_FILENAME_ARRAY, &filenames, NULL, "INPUT OUTPUT" },
  { NULL }
};
